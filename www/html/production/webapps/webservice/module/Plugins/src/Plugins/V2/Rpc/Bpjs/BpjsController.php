...
class BpjsController extends RPCResource
{
    ...
    public function listRujukanAction() {
        ...
        $data = $this->validateRequest($this->sendRequest("rujukan/listRujukan".$query));
        if(isset($data['status'])) return $data;
    ...
    {     
        $params = (array) $this->getRequest()->getQuery();
        $query = count($params) > 0 ? "?".http_build_query($params) : "";
        $data = $this->validateRequest($this->sendRequest('monitoring/klaim'.$query));
        if(isset($data['status'])) return $data;
        $list = isset($data["list"]) ? $data["list"] : null;
        $total = isset($data["count"]) ? $data["count"] : (isset($list) ? $list : 0);
        $data = $total > 0 ? $list : null;
	...
	/* Aplicares - Referensi Kamar
     */
	public function aplicaresReferensiKamarAction() {
		$data = $this->validateRequest($this->sendRequest("aplicares/referensiKamar"));		
        if(isset($data['status'])) return $data;
        $list = isset($data["list"]) ? $data["list"] : null;
        $total = isset($data["count"]) ? $data["count"] : (isset($list) ? $list : 0);
        $data = $total > 0 ? $list : null;
	...
	/* Aplicares - Ketersediaan Kamar RS
     */
	public function aplicaresKetersediaanKamarRSAction() {
		$params = (array) $this->getRequest()->getQuery();
        $query = count($params) > 0 ? "?".http_build_query($params) : "";
        $data = $this->validateRequest($this->sendRequest("aplicares/ketersediaanKamarRS".$query));
        if(isset($data['status'])) return $data;
        $list = isset($data["list"]) ? $data["list"] : null;
        $total = isset($data["count"]) ? $data["count"] : (isset($list) ? $list : 0);
        $data = $total > 0 ? $list : null;
	...
	/* Aplicares - Tempat Tidur
     */
	public function aplicaresTempatTidurAction() {
		$params = (array) $this->getRequest()->getQuery();
        $query = count($params) > 0 ? "?".http_build_query($params) : "";
        $data = $this->validateRequest($this->sendRequest("aplicares/tempatTidur".$query));
        if(isset($data['status'])) return $data;
        $list = isset($data["list"]) ? $data["list"] : null;
        $total = isset($data["count"]) ? $data["count"] : (isset($list) ? $list : 0);
        $data = $total > 0 ? $list : null;
	...
	/* Aplicares - Hapus Tempat Tidur
     */
	public function aplicaresHapusTempatTidurAction()
    {       
		$data = Json::encode($this->getPostData($this->getRequest()));
		$data = $this->validateRequest($this->sendRequest('aplicares/hapusTempatTidur', 'POST', $data));
		if(isset($data['status'])) return $data;
		return [
			'success' => true,
			'data' => $data
    ...
    {       
		...
		$data = $this->validateRequest($this->sendRequest('aplicares/mapKelas', ($method != "GET" ? 'POST' : "GET"), $data));
		if(isset($data['status'])) return $data;
