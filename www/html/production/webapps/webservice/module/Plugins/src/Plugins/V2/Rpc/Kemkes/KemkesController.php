...
class KemkesController extends RPCResource
{
     ...
     *  - kabupaten
     *  - kecamatan
     *  loadFromWs = 1 # mengambil data dari web service RS Online
     */
    public function referensiAction() {
        $query = (array) $this->getRequest()->getQuery();        
        $nama = isset($query["nama"]) ? $query["nama"] : "";
        if($nama == "") {
            $errors = (new ApiProblem(412, "Parameter nama tidak boleh kosong"))->toArray();	
			$this->response->setStatusCode($errors["status"]);
			return $errors;
        }
        unset($query["nama"]);        
        $query = count($query) > 0 ? "?".http_build_query($query) : "";        
        $result = $this->sendRequest("referensi/".$nama.$query);        
        return $this->getResultRequest($result);
    }

    /* RSOnline - Data Kebutuhan APD
     * @method dataKebutuhanApd
     * @get loadFromWs = 1 | kirim = 1
     * @post @put array(
     *    "id_kebutuhan" => int
     *    , "jumlah_eksisting" => int
     *    , "jumlah" => int
     *    , "jumlah_diterima" => int
     * )
     * @return array | mixed
     */
    public function dataKebutuhanApdAction()
    {       
		$query = (array) $this->getRequest()->getQuery();
        $query = count($query) > 0 ? "?".http_build_query($query) : "";
		$request = $this->getRequest();
        ...
        if($method != "GET") $query = "";
		$data = Json::encode($this->getPostData($request));
		$result = $this->sendRequest('data-kebutuhan-apd'.$id.$query, ($method != "GET" ? $method : "GET"), $data);
		
        return $this->getResultRequest($result);
    }

    /* RSOnline - Data Kebutuhan SDM
     * @method dataKebutuhanSdm
     * @get loadFromWs = 1 | kirim = 1
     * @post @put array(
     *    "id_kebutuhan" => int
     *    , "jumlah_eksisting" => int
     ...
     * @return array | mixed
     */
    public function dataKebutuhanSdmAction()
    {       
        $query = (array) $this->getRequest()->getQuery();
        $query = count($query) > 0 ? "?".http_build_query($query) : "";
		$request = $this->getRequest();
        ...
        if($method != "GET") $query = "";
        $data = Json::encode($this->getPostData($request));        
		$result = $this->sendRequest('data-kebutuhan-sdm'.$id.$query, ($method != "GET" ? $method : "GET"), $data);
		
        return $this->getResultRequest($result);
    }

    /* RSOnline - Data Tempat Tidur
     * @method dataTempatTidur
     * @get loadFromWs = 1 | kirim = 1
     * @post @put array(
     *    "id_tt" => int
     *    , "jumlah_ruang" => int
     ...
     * @return array | mixed
     */
    public function dataTempatTidurAction()
    {       
        $query = (array) $this->getRequest()->getQuery();
        $query = count($query) > 0 ? "?".http_build_query($query) : "";
		$request = $this->getRequest();
        ...
        if($method != "GET") $query = "";
		$data = Json::encode($this->getPostData($request));
		$result = $this->sendRequest('data-tempat-tidur'.$id.$query, ($method != "GET" ? $method : "GET"), $data);
    ...
    {       
        ...
        if($method != "GET") $query = "";
		$data = Json::encode($this->getPostData($request));
		$result = $this->sendRequest('pasien'.$id.$query, ($method != "GET" ? $method : "GET"), $data);
		
        return $this->getResultRequest($result);		
    }

    /* RSOnline - Diagnosa Pasien
     * @method diagnosaPasien
     * @get
     * @post @put array(
     *    "id" => int
     *    , "nomr" => int
     ...
     * @return array | mixed
     */
    public function diagnosaPasienAction()
    {       
        $query = (array) $this->getRequest()->getQuery();
        $query = count($query) > 0 ? "?".http_build_query($query) : "";
		$request = $this->getRequest();
        ...
        if($method != "GET") $query = "";
		$data = Json::encode($this->getPostData($request));
		$result = $this->sendRequest('diagnosa-pasien'.$id.$query, ($method != "GET" ? $method : "GET"), $data);
