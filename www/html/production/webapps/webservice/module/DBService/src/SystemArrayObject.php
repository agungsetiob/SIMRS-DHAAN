...
class SystemArrayObject extends ArrayObject
{
    ...
    public function getNotValidEntity($data, $fromMap = false) {
        ...
        foreach($this->fields as $key => $val) {
            if(is_array($val)) {
                if(isset($val["REQUIRED"])) {
                    if(!isset($data[$key])) {
                        $return[] = $val["DESCRIPTION"]." harus di isi";
                    } else {
                        if(trim($data[$key]) == null || trim($data[$key]) == "") {
                            $return[] = $val["DESCRIPTION"]." harus di isi";
                        }
                    }
                }
                if(isset($val["VALIDATOR"])) {
                    ...
                    if(!$validator->isValid($data[$key])) $return[] = $val["DESCRIPTION"]." belum benar";
                }
            }
        }
